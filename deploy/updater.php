 ...
 */
function wpme_genno_elementor_get_github_version(){
  ...
  $response = json_decode(wp_remote_retrieve_body(
    wp_remote_get(
      'https://raw.githubusercontent.com/genoo-source/wp-genoo-elementor-addon/master/version.json'
    )
  ), true);
 ...
 * Updater init
 */
function wpme_genno_elementor_updater_init($file){

  $GLOBALS['wpme_aff_downloadLink'] = 'https://github.com/genoo-source/wp-genoo-elementor-addon/archive/master.zip';
  ...
  add_filter('site_transient_update_plugins', function($transient) use ($file, $version) {
    if($transient && property_exists( $transient, 'checked') ) {
      if( $checked = $transient->checked && isset($GLOBALS['wpme_aff_plugin'])) { 
        $version = $version === null ? wpme_genno_elementor_get_github_version() : $version;
