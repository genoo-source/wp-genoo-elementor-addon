...
include_once( 'Sendy_Action_After_Submit.php' );
...
add_action( 'elementor_pro/forms/new_record', function( $record, $ajax_handler ) {
  ...
  $email_folder_id = $settings['Select'];
...
$selectvalues['form_type'] = 'EF';
  ...
  $raw_fields = $record->get( 'fields' );
    ...
    foreach($custom_ids as $custom_values)
   {
       ...
       {
           if($custom_values['custom_id'] !='message'){
          
         if($custom_values['custom_id']==$key) :
               if($custom_values['field_patten']!=''):
                          
                          $firstindex = strstr($custom_values['field_patten'], 'c00');
                        $lastindex = strstr($custom_values['field_patten'], 'date');
                         if($firstindex == true && $lastindex == true):
                                 $date=date_create($val);
                                 $date = date_format($date,"Y-m-d");
                                 $selectvalues[$custom_values['field_patten']] = $date."T".'00:00:00+00:00';
                                  update_post_meta($post->ID,$custom_values['field_patten'],$date."T".'00:00:00+00:00'); 
                           elseif($firstindex == false && $lastindex == true):
                                 $date = date_create($val);
                                 $date = date_format($date,"m/d/Y");
                                  update_post_meta($post->ID,$custom_values['field_patten'],$date); 
                                $selectvalues[$custom_values['field_patten']] = $date; 
                         elseif($custom_values['field_type']=='radio') :
                            $selectvalues[$custom_values['field_patten']] = '1';
                            update_post_meta($post->ID,$custom_values['field_patten'],'1'); 
                            elseif($custom_values['field_type']=='checkbox'):
                                $selectvalues[$custom_values['field_patten']] = '1';
                             update_post_meta($post->ID,$custom_values['field_patten'],'1'); 
                                else:
                                update_post_meta($post->ID,$custom_values['field_patten'],$val); 
                                  $selectvalues[$custom_values['field_patten']] = $val;
                                endif;
                              
                            else :
                           if($custom_values['custom_id']=='name') :
                              $custom_values['custom_id']='first_name';
                                   update_post_meta($post->ID,'first_name',$val); 
                                   $selectvalues['first_name'] = $val;
